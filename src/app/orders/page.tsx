...
export default function OrdersPage  () {
  ...
  const handleUpdate = (e:FormEvent,orderId:string) =>{
    e.preventDefault()
  ...
  return (
    <div className="p-4 lg:px-20 xl:px-40">
      <table className="w-full border-separate border-spacing-3">
        ...
        <tbody>
          {data.map((item:typeOrderData) =>(
            <tr className={`${item.status !== "delivered" ? "bg-gray-200" :"bg-red-200"}`} key={item.id}>
            ...
            <td className="hidden md:block py-6 px-1">{item.products[0].title}</td>
            {
              ...
              <td>
                <form className="flex items-center justify-center gap-4" onSubmit={(e) =>handleUpdate(e,item.id)}>
                  ...
                  <button type="submit" className="bg-red-400 p-2  rounded-full">
                    <Image src="/edit.png" alt="edit img" width={20} height={20} />
                  </button>
                </form>
              </td>
              ) :
